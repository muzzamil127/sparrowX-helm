...
def get_git_diff(service):
    r = subprocess.run(["git","diff","HEAD~1","HEAD","--",f"{service}/values.yaml"],
...
def apply_patch(section_lines, path, new_value):
    ...
    for i,line in enumerate(section_lines):
        ...
        if m:
            ...
            fv = f'"{new_value}"' if (isinstance(new_value,str) and
                 not re.match(r'^[\d.]+$',str(new_value)) and
                 str(new_value).lower() not in('true','false','null') and
                 not str(new_value).startswith('"')) else str(new_value)
            section_lines[i] = m.group(1)+fv
            print(f"  Patched [{leaf}]: {cur!r} → {fv!r}")
...
def main():
    ...
    for svc in services:
        ...
        if not key:
            print(f"WARNING: '{svc}' not in SERVICE_KEY_MAP — skipping"); continue
        ...
        if start is None: print(f"ERROR: '{key}' not found in consolidated"); continue
        ...
        prompt = textwrap.dedent(f"""
            ...
            ## Consolidated section for '{key}'
            ```yaml
            {section}
            ```

            Rules:
            - Only sync fields that have a clear equivalent in the consolidated section
            - Do NOT change lines whose value starts with * (YAML anchors)
            - Do NOT change intentionally different values (e.g. different port numbers)
            - Field names may differ (e.g. replicaCount → replicas)

            Return ONLY a JSON array: [{{"path":"image.tag","value":"v0.0.5"}}]
            Empty array [] if nothing to sync.
        """).strip()
        ...
        try:
            ...
            print(f"  Gemini error: {e}"); continue

        patches = extract_json(resp)
        if not patches: print("  No patches needed"); continue
